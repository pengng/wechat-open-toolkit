...
proto.messageMiddlewarify = function () {
  ...
  const middleware = function (req, res, next) {
    ...
    if (msg.MsgType === 'event') {
      ...
      let xml = util.buildObject(reply)
      const encode = {
        ToUserName: reply.ToUserName,
        Encrypt: this.encrypt.encode(xml)
      }
      xml = util.buildObject(encode)
      res.end(xml)
      ...
      if (msg.Content === 'TESTCOMPONENT_MSG_TYPE_TEXT') {
        ...
        const encode = {
          ToUserName: reply.ToUserName,
          Encrypt: util.encrypt.encode(xml)
        }
        xml = util.buildObject(encode)
        res.end(xml)
...
proto.getOauthUrl = function (options) {
  ...
  const newUrl = url + '?' + query + '#wechat_redirect'
  return newUrl
}
...
proto.oauthMiddlewarify = function (authorizerAppId) {
  const middleware = function (req, res, next) {
