...
const FAKE_TIMER_ID = 1 // 伪timerID
...
proto.onAuthorized = async function (data) {
    ...
    try {
        ...
        componentMap[`${AppId}/${AuthorizerAppid}`].timer = setTimeout(this.startAuthorizerAccessTokenTimer.bind(this, AppId, AuthorizerAppid), REFRESH_INTERVAL)
    } catch(err) {
        this.emit('error', err)
    }
}
...
proto.onUnauthorized = function (data) {
    ...
    delete componentMap[`${AppId}/${AuthorizerAppid}`]
}
...
proto.startAuthorizerAccessTokenTimer = async function (componentAppId, authorizerAppId) {
    ...
    clearTimeout(componentMap[`${componentAppId}/${authorizerAppId}`].timer) // 清理旧的定时器
    componentMap[`${componentAppId}/${authorizerAppId}`].timer = setTimeout(this.startAuthorizerAccessTokenTimer.bind(this, componentAppId, authorizerAppId), timeout)
}
