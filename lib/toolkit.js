...
const { WechatOpenToolkitError } = require('./error')
...
const RETRY_TIMEOUT = 1000 * 10 // 重试的超时时间，单位：毫秒。
...
const EVENT_AUTHORIZER_INFO = 'authorizer_info' // 当获取到授权方基本信息时触发
 ...
 *          host 登录授权发起页域名
 *          enableReply 默认值 false，会自动回复"success"
 ...
 *     },
 *     [componentAppId/authorizerUserName]: [componentAppId/authorizerAppId]
 * }
 */
...
function Toolkit(options) {
    ...
    list.forEach((item) => {
        ...
        componentMap[componentAppId] = Object.assign({}, item) // 对象拷贝
    })
    ...
    this.on(EVENT_AUTHORIZER_INFO, this.onReceiveAuthorizerInfo.bind(this))
...
proto.onRefreshComponentAccessToken = async function (data) {
    let { componentAppId, component_access_token } = data
    Object.assign(componentMap[componentAppId], data) // 更新 component access token

    try {
        // 获取第三方平台下已授权的授权方列表
        let { list } = await Component.getAuthorizerList(componentAppId, component_access_token)
        list.forEach(item => {
            let key = `${componentAppId}/${item.authorizer_appid}`
            if (!componentMap[key]) { // 当本地缓存的授权方列表没有时，才加入
                componentMap[key] = item
                // 定时刷新授权方的access token
                this.startAuthorizerAccessTokenTimer(componentAppId, item.authorizer_appid)
            }
        })
    } catch(err) {
        this.emit('error', err)
...
proto.onAuthorized = async function (data) {
    ...
    try {
        ...
        // 获取授权方的账号基本信息
        let { authorizer_info } = await Authorizer.getAuthorizerInfo(AppId, component_access_token, AuthorizerAppid)

        Object.assign(authorization_info, { AppId }) // 混合数据
        Object.assign(authorizer_info, { AppId, AuthorizerAppid }) // 混合数据
        
        this.emit(EVENT_AUTHORIZER_ACCESS_TOKEN, authorization_info) // 触发授权方access token更新事件
        this.emit(EVENT_AUTHORIZER_INFO, authorizer_info) // 触发授权方基本信息事件
...
// 当获取到授权方基本信息时触发
proto.onReceiveAuthorizerInfo = function (data) {
    let { AppId, AuthorizerAppid, user_name } = data
    Object.assign(componentMap[`${AppId}/${AuthorizerAppid}`], data) // 混合数据
    componentMap[`${AppId}/${user_name}`] = componentMap[`${AppId}/${AuthorizerAppid}`] // 新增别名
}
...
proto.startComponentAccessTokenTimer = async function (componentAppId) {
    let { componentAppSecret, ComponentVerifyTicket } = componentMap[componentAppId]
    let timeout = 0
...
proto.startAuthorizerAccessTokenTimer = async function (componentAppId, authorizerAppId) {
    ...
    let { authorizer_refresh_token, refresh_token } = componentMap[`${componentAppId}/${authorizerAppId}`]
    let timeout = 0
...
proto.startAuthorizerJsApiTicketTimer = async function (componentAppId, authorizerAppId) {
    let { authorizer_access_token } = componentMap[`${componentAppId}/${authorizerAppId}`]
    let timeout = 0
    ...
    return async (req, res, next) => {
        try {
            ...
            if (Encrypt) {
                ...
                // 请求主体不包含特定字段，说明数据有误
                console.warn('请求主体缺少 Encrypt 字段，消息已忽略')
            }
        ...
        try {
            ...
            if ([AUTO_TEST_MP_NAME, AUTO_TEST_MINI_PROGRAM_NAME].includes(ToUserName)) {
                if (Content === AUTO_TEST_TEXT_CONTENT) {
                    ...
                    res.end('')
                    let { authorizer_access_token } = componentMap[`${componentAppId}/${ToUserName}`]
                    await Authorizer.send(authorizer_access_token, FromUserName, 'text', { content: `${strList[1]}_from_api` })
                }
            } else {
                next && next()
            }
...
// 返回第三方授权处理的中间件
proto.auth = function (componentAppId, redirectUrl, authType = Component.AUTH_TYPE_BOTH) {
    return async (req, res) => {
        let { component_access_token } = componentMap[componentAppId]
        try {
            let { pre_auth_code } = await Component.getPreAuthCode(componentAppId, component_access_token)
            let url = Component.getAuthorizationUrl(componentAppId, pre_auth_code, redirectUrl, authType)
...
function genReplyFunc(componentAppId, encodingAESKey, token, fromUserName, toUserName, type, tpl) {
    return function () {
        ...
        let MsgSignature = wechatEncrypt.getSignature({ timestamp: TimeStamp, nonce: Nonce, msg_encrypt: Encrypt }) // 签名
