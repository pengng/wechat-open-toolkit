...
proto.onReceiveComponentVerifyTicket = function (data) {
    ...
    if (!componentMap[AppId].timer) { // 只启动一次
        this.startComponentAccessTokenTimer(AppId) // 启动定时器
    }
}
...
proto.onAuthorized = async function (data) {
    ...
    try {
        ...
        Object.assign(authorization_info, { AppId })
        // 触发授权方access token更新事件
        this.emit(EVENT_AUTHORIZER_ACCESS_TOKEN, authorization_info)
...
function genReplyFunc(componentAppId, encodingAESKey, token, fromUserName, toUserName, type, tpl) {
    return function () {
        let data = { ToUserName: toUserName, FromUserName: fromUserName, CreateTime: Date.now(), MsgType: type }
        let jsonRaw = tpl.replace(/\{(\d+)\}/g, (_, key) => arguments[key])
        Object.assign(data, JSON.parse(jsonRaw))

        let wechatEncrypt = new WechatEncrypt({ appId: componentAppId, encodingAESKey, token })
        let xml = util.buildObject(data) // js 对象转 xml 字符串
        ...
        xml = util.buildObject({ Encrypt, TimeStamp, Nonce, MsgSignature })
        this.end(xml)
    }
}
